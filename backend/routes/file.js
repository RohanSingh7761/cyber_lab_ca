...
router.get('/:fileId', authenticateToken, async (req, res) => {
    try {
        ...
        // Get file with vault information
        const query = `
            query GetFileMetadata($file_id: uuid!) {
                files_by_pk(id: $file_id) {
                    id
                    filename
                    encrypted_aes_key
                    file_hash
        ...
        `;
        const result = await executeQuery(query, { file_id: fileId });
        ...
        const hasAccess = 
            (accessCheck.file_vaults_by_pk && accessCheck.file_vaults_by_pk.creator_id === userId) || 
            (accessCheck.vault_access && accessCheck.vault_access.length > 0);
